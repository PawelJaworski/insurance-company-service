...
| header | ownership |
|---|---|
| `// GENERATED by ... DO NOT EDIT` | generator-owned, **add-only**. Regeneration inserts members the model grew and keeps everything already in the file — including hand edits — verbatim. Never *rewrite* a generated member; you may *add* one (see Ad-hoc extensions). To change what the generator emits, change the **model**. |
